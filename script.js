...
function byColor(pieces) {
  let res = []
  colors.forEach(c => {
    res.push([])
  })
  pieces.forEach(p => {
    res[colors.indexOf(p.color)].push(p)
  })
  return res
}
...
function renderScoreboard() {
  ...
  p0c.forEach((pieces, ci) => {
    pieces.forEach((p, pi) => {
      sCtx.beginPath()
      sCtx.arc(250 + 30 * (pi + 1), 30 * (ci + 1), cSize, 0, 2 * Math.PI, false);
  ...
  byColor(claimed.p0).forEach((pieces, ci) => {
    pieces.forEach((p, pi) => {
      sCtx.beginPath()
      sCtx.arc(30 * (pi + 1), 30 * (ci + 1), cSize, 0, 2 * Math.PI, false);
      sCtx.fillStyle = p.color;
      sCtx.fill();
      sCtx.lineWidth = 2;
      sCtx.strokeStyle = "gray"
  ...
  let p1c = byColor(p1)
  p1c.forEach((pieces, ci) => {
    pieces.forEach((p, pi) => {
      sCtx.beginPath()
      sCtx.arc(250 + 30 * (pi + 1), 250 + 30 * (ci + 1), cSize, 0, 2 * Math.PI, false);
      sCtx.fillStyle = p.color;
      sCtx.fill();
      sCtx.lineWidth = 2;
      sCtx.strokeStyle = "gray"
  ...
  byColor(claimed.p1).forEach((pieces, ci) => {
    pieces.forEach((p, pi) => {
      sCtx.beginPath()
      sCtx.arc(30 * (pi + 1), 250 + 30 * (ci + 1), cSize, 0, 2 * Math.PI, false);
      sCtx.fillStyle = p.color;
      sCtx.fill();
      sCtx.lineWidth = 2;
      sCtx.strokeStyle = "gray"
...
function canConnect(p1, p2) {
  res = true
  Array.prototype.concat(pieces, pawns).forEach(p3 => {
    if ((p3 == p1) || (p3 == p2)) {
      return
    }
    ...
    if (!pieceClicked) {
      if (selected.length == 2) {
        if (createNearestPawn(selected)) {
          if (curPlayer == 0) {
            claimed.p0.push(selected[0], selected[1])
          } else {
            claimed.p1.push(selected[0], selected[1])
          }
